...
def  main():
    ...
    fail=0
    for  i in range(1000):
        print(f"[{i}/1000] 获取knowledge_nn:{i}")

        knowledge_content=KnowledgeApiClient.fetch_knowledge_content(knowledge_nn=int(i+1))
        ...
        if knowledge_content and knowledge_content['content']:
            ...
            # 2.解析HTML为MarkDown
            md_content=parser.parse_html_to_markdown(knowledge_no=int(i+1), html_data=knowledge_content)
