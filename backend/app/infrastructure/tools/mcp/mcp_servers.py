...
# 百炼 MCP 已切换为 Streamable HTTP 传输（旧的 /sse 端点不再工作）
# 注意：URL 以 /mcp 结尾，不再是 /sse
...
async def run_mcp_call(
    mcp_instance: MCPServerStreamableHttp, tool_name: str, tool_args: Dict[str, Any]
):
    """
    ...
    try:
        ...
        # --- 4. 打印结果 ---
        # result 是 CallToolResult 对象，真正的文本在 result.content 里（每个元素是一个 content 块）
...
async def test_bailian_search():
    await run_mcp_call(
        ...
        tool_name="bailian_web_search",  # 替换为实际的工具名
        tool_args={"query": "广州天气"},  # 替换为实际的工具参数
    )
